...
sub main() {
    ...
    if ($action eq 'build') {
        my $jar = $Xposed::cfg->val('General', 'outdir') . '/java/AlanxpBridge.jar';
        if (!-r $jar) {
...
Values for <steps> are provided as a comma-separated list of:
  ...
  prop      Create the alanxp.prop file.
...
sub all_in_one($$;$) {
    ...
    if ($platform ne 'host' && $platform ne 'hostd') {
        ...
        create_alanxp.prop($platform, $sdk, !$silent) || return 0;
...
# Creates the /system/alanxp.prop file
sub create_alanxp.prop($$;$) {
    my $platform = shift;
    my $sdk = shift;
    my $print = shift || 0;

    should_perform_step('prop') || return 1;
    print_status("Creating alanxp.prop file...", 1);

    # Open the file
    my $coldir = Xposed::get_collection_dir($platform, $sdk);
    my $propfile = $coldir . '/files/system/alanxp.prop';
    print "$propfile\n";
...
sub create_zip($$) {
    ...
    $zip->addFile("$outdir/java/AlanxpBridge.jar", 'system/framework/AlanxpBridge.jar') || return 0;
...
sub build_java() {
    ...
    print_status('Copying APK to AlanxpBridge.jar...', 1);
    my $base = $javadir . '/app/build/outputs/apk/app-release';
    foreach my $suffix ('.apk', '-unaligned.apk', '-unsigned.apk') {
        my $file = $base . $suffix;
        if (-f $file) {
            my $target = $Xposed::cfg->val('General', 'outdir') . '/java/AlanxpBridge.jar';
            print "$file => $target\n";
            make_path(dirname($target));
            if (!copy($file, $target)) {
                print_error("Copy failed: $!");
