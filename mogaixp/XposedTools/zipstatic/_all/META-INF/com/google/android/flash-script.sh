...
if [ ! -f "system/alanxp.prop" ]; then
  echo "! Failed: Extracted file system/alanxp.prop not found!"
  exit 1
fi
...
XVERSION=$(grep_prop version system/alanxp.prop)
XARCH=$(grep_prop arch system/alanxp.prop)
XMINSDK=$(grep_prop minsdk system/alanxp.prop)
XMAXSDK=$(grep_prop maxsdk system/alanxp.prop)
...
echo "- Placing files"
install_nobackup /system/alanxp.prop                      0    0 0644
install_nobackup /system/framework/AlanxpBridge.jar       0    0 0644
...
install_nobackup  /system/lib/libalanxp_art.so            0    0 0644
if [ $IS64BIT ]; then
  ...
  install_nobackup  /system/lib64/libalanxp_art.so        0    0 0644
fi
